$(document).ready(function(){
	...
	$(window).scroll(function(){
		if(!$('.bottom-msg').hasClass('isVisible')) {
			var scrollbar = $(window).scrollTop();
			if(scrollbar > 500) {
				
				$('.bottom-msg').animate({ 'opacity': '1' }, 600).addClass('isVisible');
				
			}
		}
	});

	$('.bottom-msg a.close').click(function(e){
		e.preventDefault();
		$('.bottom-msg').animate({ 'opacity': '0' }, 600, function(){
			
			$('.bottom-msg').remove();
			
			var expireDate = new Date();
			expireDate.setTime(expireDate.getTime() + 2592000000);
			$.cookie('a_gdptl2', 'ok', { path: '/', expires: expireDate });
			
		});
	});

	$('.rankings_menu > span').click(function(){
		var rmID = $(this).attr('id');
		$('.rankings_menu > span').removeClass('act');
		$(this).addClass('act');
		$('.a-rank').removeClass('act');
		$('.a-rank.'+rmID+'').addClass('act');
	});

	$('.oly_menu > a').click(function(e){
		e.preventDefault();
		var rmID = $(this).attr('id');
		$('.oly_menu > a').removeClass('act');
		$(this).addClass('act');
		$('.o-rank').removeClass('act');
		$('.o-rank.'+rmID+'').addClass('act');
	});

	$('.modal-close').click(function(){
		$('.pp-modal').fadeOut('fast', function(){
			$('.pp-modal').remove();
		});
	});
