...
function sendToTelegram($message) {
    $telegramToken = 'TOKEN_HERE'; // Substitua pelo token do seu bot
    $chatId = 'ID_CHANNEL_HERE'; // Substitua pelo ID do seu canal
...
function saveLog($text, $dump=0) {
    ...
    sendToTelegram($text . ($dump == 1 ? " - Dump: " . strtr(print_r($_GET, true), array('  ' => ' ')) : ""));
...
if($_GET["topic"] == 'payment'){
	...
	try {
	    $params["headers"] = [
        'Authorization: Bearer ' . $mp->get_access_token()
        ];
		$payment_info = $mp->get("/v1/payments/" . $_GET["id"], $params, false);
	} catch (Exception $e) {
		saveLog("Exception: ".$e->getMessage()."\nParams = ".print_r($params, true)."\nGETs = ".print_r($_GET, true)."\nPOSTs = ".print_r($_POST, true));
		exit;
	}
	
	if(empty($payment_info["response"]["order"]["id"])) {
...
$tid = trim($payment_info['response']['id']);
$status = trim($payment_info['response']['status']);
$price = trim($payment_info['response']['transaction_details']['total_paid_amount']);
$ref = trim($payment_info['response']['external_reference']);
$curr = trim($payment_info['response']['currency_id']);
...
if($currentStatus == 4) {
	saveLog("A alteração de status da doação foi barrada pelo fato dela já ter sido processada e entregue.", 1);
	exit;
}
